...
class _PlayersScreenState extends State<PlayersScreen> {
  ...
  Future<void> _fetch() async {
    ...
    try {
      final r    = await AdminClient.get('/admin/players');
      final data = r.data;
  ...
  Future<void> _save() async {
    ...
    try {
      if (_editTarget == null) {
        await AdminClient.post('/admin/register-player', {
        ...
        await AdminClient.put(
            '/admin/update-player/${_editTarget!['id']}', body);
      }
      if (!mounted) return;
  ...
  Future<void> _delete() async {
    ...
    try {
      await AdminClient.delete(
          '/admin/delete-player/${_deleteTarget!['id']}');
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              AdminPageHeader(
                title:    'Players',
                subtitle: 'Manage player accounts',
                onAdd:    _openCreate,
                addLabel: 'New Player',
              ),
        ...
        if (_showCreate)
          CrudModal(
            title: _editTarget == null ? 'New Player' : 'Edit Player',
            ...
            child: Column(
              ...
              children: [
                ...
                ModalField(label: 'Username', controller: _u,    hint: 'player_name'),
                const SizedBox(height: 12),
                ModalField(label: 'Email',    controller: _e,    hint: 'player@example.com', type: TextInputType.emailAddress),
                const SizedBox(height: 12),
                ...
                const SizedBox(height: 12),
                ModalField(label: 'Address', controller: _addr, hint: '123 Main St', optional: true),
                const SizedBox(height: 20),
                ModalActions(
