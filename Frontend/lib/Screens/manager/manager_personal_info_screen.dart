...
class ManagerPersonalInfoScreen extends StatefulWidget {
  const ManagerPersonalInfoScreen({super.key});

  @override
  State<ManagerPersonalInfoScreen> createState() =>
      _ManagerPersonalInfoScreenState();
}

class _ManagerPersonalInfoScreenState
    extends State<ManagerPersonalInfoScreen> {
  ...
  void initState() {
    ...
    _nameCtrl    = TextEditingController(text: manager.nom     ?? auth.name);
    _emailCtrl   = TextEditingController(text: manager.profile?.user?.email ?? auth.email);
    _phoneCtrl   = TextEditingController(text: manager.phone   ?? '');
    _addressCtrl = TextEditingController(text: manager.address ?? '');
  ...
  Future<void> _save() async {
    ...
    try {
      await context.read<ManagerProvider>().updateProfile(
        context.read<ManagerService>(),
      ...
      context.read<AuthProvider>().updateProfile(name: name, phone: phone);
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 14),
                    _InfoField(label: 'Address', icon: FontAwesomeIcons.locationDot, controller: _addressCtrl),
                  ],
                ),
              ),
            ),
